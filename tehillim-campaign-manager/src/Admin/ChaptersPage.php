            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <?php for ($i = 1; $i <= ChapterTextService::MAX; $i++) : ?>
                    <details style="background:#fff;border:1px solid #ddd;border-radius:8px;padding:10px;margin:8px 0">
                        <summary><strong><?php printf(esc_html__('Tehillim chapter %s', 'tehillim-campaign-manager'), esc_html(Hebrew::chapter_label($i))); ?></strong></summary>
                        <textarea name="tcm_chapters[<?php echo esc_attr($i); ?>]" rows="8" class="large-text" style="direction:rtl;margin-top:10px"><?php echo esc_textarea($chapters[$i] ?? ''); ?></textarea>
                    </details>
                <?php endfor; ?>
                <p><button type="submit" class="button button-primary"><?php esc_html_e('Save all chapters', 'tehillim-campaign-manager'); ?></button></p>
            </form>
