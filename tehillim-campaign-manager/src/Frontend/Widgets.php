...
final class Widgets implements Registerable {
    ...
    public function progress_bar($atts) {
        $atts = shortcode_atts(array('id' => 0, 'label' => '1'), $atts);
        $id   = $this->id($atts);
        if (!$id) {
            return '';
        }
        $s     = $this->stats->for_campaign($id);
        $label = '0' !== (string) $atts['label']
            ? '<div class="tcm-progress-label">' . esc_html(sprintf(__('Progress: %s%%', 'tehillim-campaign-manager'), $s['percent'])) . '</div>'
            : '';
    ...
    public function stats_box($atts) {
        ...
        return '<div class="tcm-wrap"><div class="tcm-card"><ul class="tcm-mini-stats">'
            ...
            . '<li class="tcm-mini-stat"><strong>' . esc_html($this->assignments->participant_count($id)) . '</strong>' . esc_html__('Participants', 'tehillim-campaign-manager') . '</li>'
    ...
    public function cta($atts) {
        ...
        $threshold = (int) ( Options::get('finish_wave_threshold', 20) ?: 20 );
    ...
    public function urgency($atts) {
        ...
        if ($remaining <= $threshold) {
            return '<span class="tcm-badge tcm-urgent">' . esc_html(sprintf(__('Only %d chapters left to finish!', 'tehillim-campaign-manager'), $remaining)) . '</span>';
        }
        return '<span class="tcm-badge">' . esc_html(sprintf(__('%d free chapters remaining', 'tehillim-campaign-manager'), $remaining)) . '</span>';
    }
    ...
    public function data($atts) {
        $atts = shortcode_atts(
            array('id' => 0, 'field' => 'percent', 'prefix' => '', 'suffix' => '', 'if_less' => '', 'text' => ''),
            $atts
        );
        $id = $this->id($atts);
        if (!$id) {
