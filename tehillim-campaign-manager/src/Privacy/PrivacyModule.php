    ...
    public function export($email, $page = 1) {
        ...
        foreach (( new AssignmentsRepository() )->all_by_email($email) as $row) {
            $items[] = array(
                ...
                'data'        => array(
                    array('name' => __('Campaign', 'tehillim-campaign-manager'), 'value' => get_the_title((int) $row->campaign_id)),
                    array('name' => __('Chapter', 'tehillim-campaign-manager'), 'value' => Hebrew::chapter_label((int) $row->chapter_number)),
                    array('name' => __('Status', 'tehillim-campaign-manager'), 'value' => $row->status),
                    array('name' => __('Name', 'tehillim-campaign-manager'), 'value' => (string) $row->participant_name),
                    array('name' => __('Phone', 'tehillim-campaign-manager'), 'value' => (string) $row->participant_phone),
                ),
            );
        }
        ...
        foreach (( new SubscribersRepository() )->all_by_email($email) as $sub) {
            $items[] = array(
                ...
                'data'        => array(
                    array('name' => __('List', 'tehillim-campaign-manager'), 'value' => (string) $sub->list_key),
                    array('name' => __('Channel', 'tehillim-campaign-manager'), 'value' => (string) $sub->channel),
                    array('name' => __('Phone', 'tehillim-campaign-manager'), 'value' => (string) $sub->phone),
                    array('name' => __('Status', 'tehillim-campaign-manager'), 'value' => (string) $sub->status),
                ),
            );
        }
